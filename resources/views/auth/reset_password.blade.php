        ...
        <div class="md:w-1/2 lg:w-1/4 bg-green-50 p-4 rounded mt-4 md:mt-12 lg:mt-32">
            ...
            <form method="post" action="{{route('password.update', ['plan' => $plan])}}">
                ...
                <div>
                    ...
                    <input id="password" name="password" type="password" class="@error('password') border-red-500 @enderror w-full block text-black p-1 text-lg mb-2 border rounded" value="">
                    @error('password')
                    <div class="text-red-500 text-xs italic">{{ $message }}</div>
                    @enderror
                </div>
