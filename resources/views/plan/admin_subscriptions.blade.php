        ...
        @foreach($plan->shifts as $index => $shift)
            ...
            @endif
              @foreach($shift->subscriptions as $subscription)
                    ...
                    @endif
                        ...
                        <td class="px-1">
                            <a href="{{route('plan.shift.subscription.edit',  ['plan' => $plan, 'shift' => $shift, 'subscription' => $subscription])}}" class="mt-2 my-button">
                        ...
                        <td class="px-0">
                            <form method="post" action="{{route('plan.shift.subscription.destroy', ['plan' => $plan, 'shift' => $shift, 'subscription' => $subscription])}}">
                                @method('delete')
                                @csrf
                                <button type="submit" class="mt-2 my-button">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" viewBox="0 0 20 20" fill="currentColor">
            ...
            @if($shift->subscriptions->count() < $shift->team_size)
                ...
                @else
                  <td colspan=3></td>
                @endif
              <td class="px-4 bg-red-100" colspan="6">
                <b>
